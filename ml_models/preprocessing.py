...
def encode_features(X_train, X_test):
    ...
    if len(multi_cols) > 0:
        ...
        encoded_train_df = pd.DataFrame(
            encoded_train,
            columns=encoder.get_feature_names_out(multi_cols),
            index=X_train.index
        )
...
def handle_skewness(X_train, X_test):
    ...
    if len(skewed_cols) > 0:
        ...
        X_train[skewed_cols] = transformer.transform(
            X_train[skewed_cols]
        )

        X_test[skewed_cols] = transformer.transform(
            X_test[skewed_cols]
        )
...
def scale_features(
    ...
    X_train[numerical_cols] = scaler.transform(
        X_train[numerical_cols]
    )
...
def remove_low_variance_features(
    ...
    X_train_new = selector.transform(
        X_train
    )

    X_test_new = selector.transform(
        X_test
    )
...
def preprocess_data(
    ...
    X_train, X_test = handle_skewness(
        X_train,
        X_test
    )
    ...
    X_train, X_test = remove_low_variance_features(
        X_train,
        X_test
    )
